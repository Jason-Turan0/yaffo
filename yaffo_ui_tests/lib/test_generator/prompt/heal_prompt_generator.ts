...
export class HealPromptGenerator {
    ...
    buildSystemPrompt(): string {
        return [
            ...
            "    10. Before hard-coding seeded database IDs or duplicating lookup logic, inspect existing shared helper modules",
            "        under generated_tests/_support/ and import/reuse them.",
    ...
    buildAnalysisPrompt(context: HealContext, allowedDirs: string[], outputSchema?: string): string {
        ...
        const blocks = [
            ...
            "    - test_code_defect: The test code is broken. Use the spec goals as the source of truth —",
            "      if the spec goals can be achieved with the current running application, the test is wrong.",
            "    - application_regression: The application has a real bug that prevents the spec goals from being achieved.",
            ...
            "    3. Use Playwright tools to interact with the live site and verify whether the spec goals can be achieved.",
            "    4. Compare the spec goals against what the application actually supports.",
