...
export const describeSandboxFacts = (facts: SandboxFacts): string => {
    ...
    return [
        ...
        "```",
        "",
        "`mediaIdByFilename` is given to **both** a shot's `goto` and to `flows`, so a",
        "flow that drives the detail view reaches an item the same way:",
        "",
        "```typescript",
        "flows: async ({visit, page, mediaIdByFilename}) => {",
        "    const id = await mediaIdByFilename(PRIMARY_DETAIL_IMAGE);",
        "    await visit(`/media/view/${id}`);",
        "},",
        "```",
        "",
