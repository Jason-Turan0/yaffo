...
from yaffo.db.models import (
    MediaItem, Job,
    JOB_STATUS_COMPLETED, MEDIA_STATUS_INDEXED,
)
...
def test_full_pipeline_imports_then_indexes(immediate_db, tmp_path):
    ...
    with _session(engine) as session:
        ...
        assert all(p.status == MEDIA_STATUS_INDEXED for p in media_items)
...
def test_empty_import_still_indexes_existing(immediate_db, tmp_path):
    ...
    with _session(engine) as session:
        ...
        assert index_job.status == JOB_STATUS_COMPLETED
        assert all(p.status == MEDIA_STATUS_INDEXED for p in session.query(MediaItem).all())
...
def test_nothing_to_do_completes_both_jobs(immediate_db, tmp_path):
    ...
    with _session(engine) as session:
        for f in files:
            session.add(MediaItem(full_file_path=f, status=MEDIA_STATUS_INDEXED))
        session.commit()
