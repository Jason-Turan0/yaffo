...
def test_delete_photos_removes_photo_and_all_dependents(session):
    ...
    keep = Photo(full_file_path="/lib/keep.jpg")
    person = Person(name="Alice")
    label = ClassificationLabel(name="beach")
    session.add_all([photo, keep, person, label])
    session.flush()

    face = Face(photo_id=photo.id, full_file_path="/thumbs/face1.jpg")
    session.add(face)
    session.flush()
    session.add_all([
        Tag(photo_id=photo.id, tag_name="trip", tag_value="maui"),
        PhotoLabel(photo_id=photo.id, label_id=label.id, confidence=0.9),
    ...
    assert session.query(Face).filter_by(photo_id=photo_id).count() == 0
    assert session.query(Tag).filter_by(photo_id=photo_id).count() == 0
    assert session.query(PhotoLabel).filter_by(photo_id=photo_id).count() == 0
    ...
    assert session.get(Photo, keep_id) is not None
