"""Unit tests for the mutating move_media_item host action and data_query row
...
def test_move_photo_moves_within_media_dir(monkeypatch, tmp_path):
    ...
    _patch(monkeypatch, "yaffo.background_tasks.automation_sandbox.automation_actions."
           "media_repository.get_media_item_path", lambda s, pid: str(src))
    _patch(monkeypatch, "yaffo.background_tasks.automation_sandbox.automation_actions."
           "media_repository.update_media_item_path", lambda s, pid, path: saved.update(path=path))
    ...
    automation_actions.move_media_item(object(), 1, "GUID", "2024/06")
...
def test_move_photo_refuses_escape(monkeypatch, tmp_path):
    ...
    automation_actions.move_media_item(object(), 1, "GUID", "../../etc")
...
def test_move_photo_noop_when_destination_equals_source(monkeypatch, tmp_path):
    ...
    automation_actions.move_media_item(object(), 1, "GUID", "2024")
...
def test_assign_faces_filters_unknown_persons(monkeypatch, emitted_events):
    ...
    _patch(monkeypatch, _ACTIONS + "media_repository.get_media_item_ids_for_faces",
    ...
    assert emitted_events == [("media_modified", {"media_item_ids": [100]})]
...
def test_delete_photos_trashes_files_then_removes_rows(monkeypatch, tmp_path, emitted_events):
    ...
    _patch(monkeypatch, _ACTIONS + "media_repository.get_paths_by_ids",
           lambda s, ids: {1: str(a), 2: str(b)})
    monkeypatch.setattr(automation_actions.send2trash, "send2trash", lambda p: trashed.append(p))
    _patch(monkeypatch, _ACTIONS + "media_repository.delete_media_items", _capture_delete)

    automation_actions.delete_media_items(object(), [1, 2])
...
def test_delete_photos_keeps_row_when_trash_fails(monkeypatch, tmp_path):
    ...
    _patch(monkeypatch, _ACTIONS + "media_repository.get_paths_by_ids", lambda s, ids: {1: str(a)})
    ...
    automation_actions.delete_media_items(object(), [1])
...
def test_delete_photos_unlinks_returned_face_thumbnails(monkeypatch, tmp_path):
    ...
    thumb.write_text("t")
    _patch(monkeypatch, _ACTIONS + "media_repository.get_paths_by_ids", lambda s, ids: {})
    _patch(monkeypatch, _ACTIONS + "media_repository.delete_media_items", lambda s, ids: [str(thumb)])
...
def test_move_photos_moves_all_and_commits_once(monkeypatch, tmp_path, emitted_events):
    ...
    _patch(monkeypatch, _ACTIONS + "media_repository.get_media_item_path", lambda s, pid: str(srcs[pid]))
    _patch(monkeypatch, _ACTIONS + "media_repository.update_media_item_path",
    ...
    automation_actions.move_media_items(session, [
        {"media_item_id": 1, "media_dir_id": "G", "target_path": "2024"},
        {"media_item_id": 2, "media_dir_id": "G", "target_path": "2024"},
    ])
...
def test_rename_files_renames_all_and_commits_once(monkeypatch, tmp_path):
    ...
    _patch(monkeypatch, _ACTIONS + "media_repository.get_media_item_path", lambda s, pid: str(src))
    _patch(monkeypatch, _ACTIONS + "media_repository.update_media_item_path",
           lambda s, pid, path: saved.append(path))

    session = _CommitCountingSession()
    automation_actions.rename_files(session, [{"media_item_id": 1, "new_name": "new.jpg"}])
...
def test_enrich_photo_rows_adds_media_dir_id_and_relative_path(monkeypatch):
    _patch(monkeypatch, "yaffo.background_tasks.automation_sandbox.media_dirs."
           "media_repository.get_paths_by_ids",
    ...
    rows = media_dirs.enrich_media_rows(object(), [{"id": 1, "year": 2024}, {"id": 2}])
