...
def test_borrows_nearest_in_time_coordinates(monkeypatch):
    ...
    updated = mod._geotag_from_neighbors(_FakeSession(), [1], max_minutes=30)

    assert updated == [1]
    assert (target.latitude, target.longitude) == (48.8584, 2.2945)


def test_skips_when_nearest_is_outside_window(monkeypatch):
    target = _photo(1, "2022-07-03T10:04:00")
...
def test_picks_closest_of_before_and_after(monkeypatch):
...
def test_candidates_frozen_no_chaining(monkeypatch):
    ...
    updated = mod._geotag_from_neighbors(_FakeSession(), [1, 2], max_minutes=5)
...
def test_no_targets_is_noop(monkeypatch):
    _stub(monkeypatch, targets=[], gps=[_gps("2022-07-03T10:00:00", 1.0, 1.0)])
    session = _FakeSession()

    assert mod._geotag_from_neighbors(session, [1], max_minutes=30) == []
    assert session.commits == 0


def test_no_gps_photos_is_noop(monkeypatch):
    target = _photo(1, "2022-07-03T10:00:00")
    _stub(monkeypatch, targets=[target], gps=[])

    assert mod._geotag_from_neighbors(_FakeSession(), [1], max_minutes=30) == []
    assert target.latitude is None


def test_unparseable_target_date_is_skipped(monkeypatch):
...
def test_copies_location_name_from_matched_source(monkeypatch):
    target = _photo(1, "2022-07-03T10:04:00")
    _stub(monkeypatch, targets=[target], gps=[
...
def test_no_location_name_when_source_has_none(monkeypatch):
    target = _photo(1, "2022-07-03T10:04:00")
    _stub(monkeypatch, targets=[target], gps=[_gps("2022-07-03T10:01:00", 48.8584, 2.2945)])

    mod._geotag_from_neighbors(_FakeSession(), [1], max_minutes=30)

    assert target.location_name is None


def test_does_not_overwrite_existing_location_name(monkeypatch):
