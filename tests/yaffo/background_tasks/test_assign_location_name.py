...
def _photo(photo_id, lat, lon, location_name=None):
    return SimpleNamespace(id=photo_id, latitude=lat, longitude=lon, location_name=location_name)


def _stub_repo(monkeypatch, *, batch, named):
    monkeypatch.setattr(mod.photos_repository, "get_photos_with_coords", lambda s, ids: batch)
    monkeypatch.setattr(mod.photos_repository, "get_named_coordinates", lambda s: list(named))
...
def test_reuse_disabled_goes_straight_to_geocode(monkeypatch, progress_reporter):
    ...
    named_calls = []
    monkeypatch.setattr(mod.photos_repository, "get_photos_with_coords", lambda s, ids: [photo])
    monkeypatch.setattr(
        mod.photos_repository, "get_named_coordinates",
