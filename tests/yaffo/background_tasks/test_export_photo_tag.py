...
def test_exports_both_tags(tmp_path, monkeypatch, progress_reporter):
    ...
    media_item = _photo(tmp_path, location="Paris", people=[["Alice"], ["Bob"]])
    session = _FakeSession([media_item])
...
def test_location_only(tmp_path, monkeypatch, progress_reporter):
    calls = _patch_writes(monkeypatch)
    media_item = _photo(tmp_path, location="Paris", people=[["Alice"]])
    written = mod._export_tags(_FakeSession([media_item]), progress_reporter, [1], export_location=True, export_people=False)
    assert written == 1
...
def test_people_only(tmp_path, monkeypatch, progress_reporter):
    calls = _patch_writes(monkeypatch)
    media_item = _photo(tmp_path, location="Paris", people=[["Alice"]])
    written = mod._export_tags(_FakeSession([media_item]), progress_reporter, [1], export_location=False, export_people=True)
    assert written == 1
...
def test_neither_enabled_is_noop(tmp_path, monkeypatch, progress_reporter):
    calls = _patch_writes(monkeypatch)
    media_item = _photo(tmp_path, location="Paris", people=[["Alice"]])
    assert mod._export_tags(_FakeSession([media_item]), progress_reporter, [1], export_location=False, export_people=False) == 0
    assert calls == []
...
def test_missing_file_is_skipped(tmp_path, monkeypatch, progress_reporter):
    calls = _patch_writes(monkeypatch)
    media_item = SimpleNamespace(full_file_path=str(tmp_path / "gone.jpg"), location_name="X", faces=[])
    assert mod._export_tags(_FakeSession([media_item]), progress_reporter, [1], export_location=True, export_people=True) == 0
    assert calls == []
    assert progress_reporter.run_with_progress_calls == [[media_item]]
...
def test_people_names_deduped_and_sorted(tmp_path, monkeypatch, progress_reporter):
    calls = _patch_writes(monkeypatch)
    media_item = _photo(tmp_path, location=None, people=[["Bob", "Alice"], ["Alice"]])
    mod._export_tags(_FakeSession([media_item]), progress_reporter, [1], export_location=False, export_people=True)
...
def test_exports_labels_as_keywords(tmp_path, monkeypatch, progress_reporter):
    calls = _patch_writes(monkeypatch)
    media_item = _photo(tmp_path, location="Paris", people=[["Alice"]], labels=["beach", "sunset"])
    written = mod._export_tags(_FakeSession([media_item]), progress_reporter, [1], False, False, export_labels=True)
    assert written == 1
...
def test_exports_custom_tags_as_keywords(tmp_path, monkeypatch, progress_reporter):
    calls = _patch_writes(monkeypatch)
    media_item = _photo(tmp_path, location=None, people=[], tags=[("rating", "5"), ("favorite", None)])
    mod._export_tags(_FakeSession([media_item]), progress_reporter, [1], False, False, export_custom_tags=True)
...
def test_labels_and_custom_tags_combine_into_keywords(tmp_path, monkeypatch, progress_reporter):
    calls = _patch_writes(monkeypatch)
    media_item = _photo(tmp_path, location=None, people=[], labels=["beach"], tags=[("trip", "maui")])
    mod._export_tags(_FakeSession([media_item]), progress_reporter, [1], False, False, export_labels=True, export_custom_tags=True)
...
def test_labels_off_means_no_keywords(tmp_path, monkeypatch, progress_reporter):
    calls = _patch_writes(monkeypatch)
    media_item = _photo(tmp_path, location="Paris", people=[["Alice"]], labels=["beach"], tags=[("x", "y")])
    mod._export_tags(_FakeSession([media_item]), progress_reporter, [1], export_location=True, export_people=True)
...
def test_exports_favorite_keyword_when_set(tmp_path, monkeypatch, progress_reporter):
    calls = _patch_writes(monkeypatch)
    media_item = _photo(tmp_path, location=None, people=[], favorite=True)
    written = mod._export_tags(_FakeSession([media_item]), progress_reporter, [1], False, False, export_favorite=True)
    assert written == 1
...
def test_favorite_unset_writes_no_keyword(tmp_path, monkeypatch, progress_reporter):
    ...
    media_item = _photo(tmp_path, location=None, people=[], favorite=None)
    mod._export_tags(_FakeSession([media_item]), progress_reporter, [1], False, False, export_favorite=True)
...
def test_handler_enqueues_for_event_photos(monkeypatch):
    ...
    mod.enqueue_export_photo_tag(SimpleNamespace(id=3), SimpleNamespace(media_item_ids=[11, 12]))
