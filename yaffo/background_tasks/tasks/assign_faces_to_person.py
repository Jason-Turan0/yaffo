...
from yaffo.db.models import PersonFace, FACE_STATUS_ASSIGNED, Person, Face, EVENT_PHOTO_MODIFIED, MediaItem, \
...
def assign_faces_to_person(person_id: int, face_ids: list[int]):
    ...
    try:
        ...
        photo_ids = [face.media_item_id for face in faces]
        session.query(MediaItem).filter(MediaItem.id.in_(photo_ids)).update(
            {MediaItem.status: PHOTO_STATUS_INDEXED}, synchronize_session=False
        )
        session.commit()
        update_person_embedding(person_id, session)
        emit_event(EVENT_PHOTO_MODIFIED, {"media_ids": list(set(photo_ids))})
