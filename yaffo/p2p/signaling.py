"""Persistent signaling connection to the hub, plus the two halves of the
relay-first call flow (the Tailscale/DCUtR pattern):
  ...
  relay HELLO, then a pinned QUIC exchange *via the relay* — the call works
  from here on no matter what the NATs do.
  punch toward B; if it lands, a second pinned QUIC exchange *directly* to
  B's NAT-mapped address — same certificate check on both paths.

Presence is the WebSocket being open; there is nothing to register and
nothing to poll. The hub only forwards blobs — every trust decision stays
end-to-end (fingerprint pinning inside the QUIC handshake).
"""
...
from yaffo.p2p.relay import new_session_token
...
class HubClient:
    ...
    def _dispatch(self, message: dict) -> None:
        ...
        if kind == "challenge":
            # The hub authenticates the socket before forwarding anything:
            # it sends a nonce, we prove possession of the device key
            # (device IDs are self-authenticating hashes of the pubkey, so
    ...
    async def notify_revoked(self, peer_device_id: str, signed_notice: dict) -> None:
        ...
        next denied request instead) — revocation is enforced locally
        either way.
        """
    ...
    async def call(
        ...
        choice for quick request/response interactions, where waiting out a
        punch that may never land would just stall the answer we already
        have.
        """
